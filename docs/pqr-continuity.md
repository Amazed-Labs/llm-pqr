...
A consumer may deliberately run a request in an isolated environment when that is a separately defined requirement — for example, bounded analysis of untrusted content. That isolation must be explicit in the consumer's runtime policy (for example, `context_mode: isolated`); it is never implied by the selected provider, model, locality, cost, or capability score.
