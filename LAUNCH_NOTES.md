...
**Verification note (2026-08-17):** the Hermes policy-routing pilot is private and independently evolved; it does not import LLM-PQR as a runtime dependency. Its focused ingress-router suite is **82/82 passing** at private commit `6a5638594c`. Do not call it a public integration, production deployment, benchmark, or preference-dominance sweep.
...
- A private Hermes policy-routing pilot exists. It independently explores the same bounded-routing questions as LLM-PQR; it is not a runtime dependency or public integration. The pilot has **82/82 focused ingress-router tests passing**, covering caller assertions, fail-closed required-route behavior, rewrite monotonicity, and content-free route logging.
...
A practical policy-routing experiment exists in private: a Hermes Agent ingress-router pilot explores the same bounded-routing questions (private → authorized local; consequential → required capability floor; ordinary → weighted eligible choice). It is not a public integration or runtime dependency. The pilot's focused ingress-router suite is **82/82 passing**. Happy to write up the design separately if there's interest.
...
It's early (alpha). Separately, I have a private Hermes policy-routing pilot exploring the same constraints: **82/82 focused ingress-router tests pass**, including caller assertions, required-route fail-closed behavior, and monotonic rewrite upgrades. It is not a public integration or benchmark. Happy to talk about either direction.
...
**Post 5:**
A separate private Hermes policy-routing pilot is exploring the same bounded-routing questions. At the current private commit: **82/82 focused ingress-router tests pass**.

The pilot supports caller assertions, required-route fail-closed behavior, and monotonic rewrite upgrades. It is not a public integration, benchmark, or production claim.
